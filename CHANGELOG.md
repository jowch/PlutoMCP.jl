...
- **`allow_execution(run_notebook=true)` left `pending_run` set:** the notebook run bypassed the run tools' path, so cells edited in safe preview were still reported as edited-but-unrun (`list_notebooks`, mutation receipts) after they ran. It now runs through the same path and clears `pending_run` for the cells that complete.
